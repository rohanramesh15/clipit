...
- ⚠️ **Not yet confirmed**: Authentication → URL Configuration — **Site URL** should be `https://theclipitapp.com`, and **Redirect URLs** allow-list should include `http://localhost:5173/**`, `https://theclipitapp.com/**`, `https://www.theclipitapp.com/**`. Without this, Supabase will reject the OAuth redirect during Step 5's Google sign-in test. Codex/user should verify this is set before testing Google sign-in.
...
- **Frontend NOT yet cut over** — `.env.production` already has the right `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY` values from Codex's commit, but nothing has been pushed/deployed to the actual frontend host yet. Also still unresolved: user mentioned moving the frontend to Fly.io with a new domain instead of the current Vercel/`theclipitapp.com` setup — need to clarify this before deploying frontend changes, since it affects which URL goes into Supabase's production Site URL / Redirect URLs.
