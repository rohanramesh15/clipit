...
interface FlashCard {
  ...
  timestamp: number;
  video_id: string;
  difficulty: 'beginner' | 'intermediate' | 'advanced' | 'very_advanced';
  rank: number;
}
...
const DIFFICULTY_COLORS: Record<string, string> = {
  ...
  very_advanced: 'text-red-400 bg-red-500/10 border-red-500/30',
};
...
export function FlashcardsPage() {
  ...
  const loadFlashcards = useCallback(async (videoId: string, videoTitle: string) => {
    ...
    try {
      ...
      // 3. Get flashcard data with sentences + timestamps
      ...
      if (!fcRes.ok) throw new Error('flashcards');
  ...
  useEffect(() => {
    async function bootstrap() {
      try {
        ...
        // Auto-pick most recently tracked video
        const first = vids[0];
        await loadFlashcards(first.video_id, first.title);
      } catch {
        setLoadState('error');
      }
    }
  ...
  if (loadState === 'error') {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 px-4">
        ...
        <button
          onClick={() => selectedVideoId
            ? loadFlashcards(selectedVideoId, selectedVideoTitle)
  ...
  if (loadState === 'no-korean') {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 px-4">
        ...
        <p className="text-primary font-semibold">No Korean vocabulary found</p>
        <p className="text-secondary text-sm text-center max-w-sm">
          No intermediate-level Korean words were found in this video.
        </p>
        {videos.length > 1 && (
          <button
  ...
  return (
    <div className="min-h-screen flex flex-col items-center max-w-md mx-auto px-4 py-8 md:py-10">
      ...
      <AnimatePresence>
        {showVideoPicker && videos.length > 1 && (
          <motion.div
            ...
            {videos.map(v => (
              <button
                ...
                onClick={() => {
                  ...
                  loadFlashcards(v.video_id, v.title);
                }}
      ...
      <div className="w-full">
        {/* YouTube clip — iframe remounts on card change to seek to timestamp */}
            ...
            <motion.div
              ...
              <motion.div
                ...
                <div
                  ...
                  <span className={`text-[10px] font-bold tracking-widest uppercase px-2.5 py-1 rounded-full border mb-5 ${
                    currentCard ? DIFFICULTY_COLORS[currentCard.difficulty] || DIFFICULTY_COLORS.intermediate : ''
                  }`}>
                    {currentCard?.difficulty?.replace('_', ' ') || 'word'}
                  </span>
